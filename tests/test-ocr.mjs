...
//             tests/samples/headquarters-scanned.pdf   image-only scan of headquarters.pdf p1
...
const fixturesReady = () =>
  fs.existsSync(SCANNED_PDF) && fs.existsSync(SCAN_JPEG) && fs.existsSync(SCHEDULE_JPEG) &&
  fs.existsSync(BLANK_PDF) && fs.existsSync(ROTATED_PDF);
const info = {};
...
await check("isProbablyScanned: the real sheet vs the scanned fixture", async () => {
  ...
  assert.ok(real.length > 100, `headquarters.pdf page 1 should have a text layer, got ${real.length}`);
  ...
  return `${real.length} text items in headquarters.pdf vs ${scanned.length} in the scan`;
});
