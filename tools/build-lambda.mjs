...
const SMOKE_SOURCE = `// generated by tools/build-lambda.mjs — parses the synthetic sample from this staged
...
const head = Buffer.from(
  '--' + boundary + '\\r\\nContent-Disposition: form-data; name="files"; filename="sample-plan.pdf"\\r\\nContent-Type: application/pdf\\r\\n\\r\\n',
  "utf8"
);
...
if (rooms !== 159) { console.error("  staged tree parsed " + rooms + " rooms, expected 159"); process.exit(3); }
`;
...
`);
  ...
  console.log(`staged smoke check (parses tests/samples/sample-plan.pdf through the staged tree)...`);
  smokeTestStaged(path.join(ROOT, "tests", "samples", "sample-plan.pdf"));
